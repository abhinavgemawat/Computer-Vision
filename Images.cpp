...
void ImagesDemos( Mat& image1, Mat& image2, Mat& logo_image, Mat& people_image )
{
	...
	cvtColor(smaller_image, smaller_image_display, COLOR_GRAY2BGR);
	cvtColor(resized_image, resized_image_display, COLOR_GRAY2BGR);
	cvtColor(two_bit_image, two_bit_image_display, COLOR_GRAY2BGR);
	...
	cvtColor(input_planes[2], channel1_display, COLOR_GRAY2BGR);
	cvtColor(input_planes[1], channel2_display, COLOR_GRAY2BGR);
	cvtColor(input_planes[0], channel3_display, COLOR_GRAY2BGR);
	...
	Mat yuv_image;
	cvtColor(smaller_image, yuv_image, COLOR_BGR2YUV);
	split(yuv_image,input_planes);
	cvtColor(input_planes[0], channel1_display, COLOR_GRAY2BGR);
	cvtColor(input_planes[1], channel2_display, COLOR_GRAY2BGR);
	cvtColor(input_planes[2], channel3_display, COLOR_GRAY2BGR);
	...
	Mat hls_image;
	cvtColor(smaller_image, hls_image, COLOR_BGR2HLS);
	...
	Mat lab_image;
	cvtColor(smaller_image, lab_image, COLOR_BGR2Lab);
	vector<Mat> lab_planes(3);
	split(lab_image,lab_planes);
	cvtColor(lab_planes[0], channel1_display, COLOR_GRAY2BGR);
	cvtColor(lab_planes[1], channel2_display, COLOR_GRAY2BGR);
	cvtColor(lab_planes[2], channel3_display, COLOR_GRAY2BGR);
	...
	Mat noise_test4 = noise_test.clone();
	Mat noise_test42;
	...
	bilateralFilter(noise_test4, noise_test42, 9, 100, 100);
	...
	output4 = JoinImagesHorizontally(output3, "", noise_test42, "Bilateral filter", 4);
	...
	output2 = JoinImagesHorizontally(output1,"",noise_test2,"Gaussian filtered",4);
	output3 = JoinImagesHorizontally(output2,"",noise_test3,"Median filtered",4);
	output5 = JoinImagesHorizontally(output3, "", noise_test42, "Bilateral filter", 4);
	output2 = JoinImagesVertically(output4,"",output5,"",4);
	output1 = JoinImagesHorizontally(smaller_image,"Original Image",output2,"",4);
	imshow("Noise and Smoothing", output1);
	c = cv::waitKey();
    cv::destroyAllWindows();

	Mat image3, image4, image5;
	pyrDown(image1, image3);
	pyrDown(image3, image4);
	pyrDown(image4, image5);
	output1 = JoinImagesVertically(image3, "", image1, "", 4);
	output2 = JoinImagesVertically(image4, "", output1, "", 4);
	output3 = JoinImagesVertically(image5, "", output2, "", 4);
	imshow("Image Pyramid", output3);
	c = cv::waitKey();
	cv::destroyAllWindows();
