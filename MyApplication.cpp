...
void AnnotatedImages::CompareObjectsWithGroundTruth(AnnotatedImages& training_images, AnnotatedImages& ground_truth, ConfusionMatrix& results)
{
	...
	for (int ground_truth_image_index = 0; ground_truth_image_index < ground_truth.annotated_images.size(); ground_truth_image_index++)
	{
		...
		if (current_annotated_recognition_image != NULL)
		{
			...
			{
				Mat smaller_image;
				resize(*display_image, smaller_image, Size(display_image->cols / 6, display_image->rows / 6));
...
vector<Point> outputImageContour;

void ImageWithBlueSignObjects::LocateAndAddAllObjects(AnnotatedImages& training_images)
{
	AnnotatedImages inputImage = training_images;
	ObjectAndLocation* currentObject = NULL;
	currentObject = (inputImage.annotated_images[0])->getObject(0);
	Mat output = currentObject->getImage();
	Mat resizedImage;
	Mat postThreshold;
	cv::Mat outputImage;

	resize(image, resizedImage, Size(image.cols / 4, image.rows / 4));
	cv::cvtColor(resizedImage, outputImage, COLOR_BGR2HSV);
	...
	threshold(S, postThreshold, 35, 255, cv::THRESH_BINARY_INV || cv::THRESH_OTSU);
	//imshow(filename, postThreshold);


	findContours(postThreshold, contours, hierarchy, RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
	cv::Mat clonedImage = resizedImage.clone();

	int i = 0;
	while(i<contours.size())
	{
	
		approxPolyDP(contours[i], outputImageContour, arcLength(contours[i], true) * 0.02, true); // approxPolyDP
		double areaDetected = contourArea(contours[i]);
				
		// for cropping
		if (outputImageContour.size() == 4 && areaDetected > 1000 && areaDetected < 10000) // filtering for removing unwanted contours
		{
			int xCoordinateMin = outputImageContour[0].x;
			int yCoordinateMin = outputImageContour[0].y;
			int xCoordinateMax = outputImageContour[0].x;
			int yCoordinateMax = outputImageContour[0].y;
			
			int a = 0;
			while (a < 4 ) {
				if ( xCoordinateMin - outputImageContour[a].x > 0) {
					xCoordinateMin = outputImageContour[a].x;
					
				}
				if (yCoordinateMin - outputImageContour[a].y > 0) {
					yCoordinateMin = outputImageContour[a].y;
					
				}
				if (outputImageContour[a].x - xCoordinateMax > 0) {
					xCoordinateMax = outputImageContour[a].x;
					
				}
				if (outputImageContour[a].y - yCoordinateMax > 0) {
					yCoordinateMax = outputImageContour[a].y;
					
				}
				//line(clonedImage, outputImageContour[a], outputImageContour[(a + 1) % 4], Scalar(0, 0, 255), 2, LINE_AA, 0);
				a++;
			}
			int width = xCoordinateMax - xCoordinateMin;
			int length = yCoordinateMax - yCoordinateMin;
			Mat croppedImage;
				cv::Rect RegionOfInterest(xCoordinateMin, yCoordinateMin, width, length);
				cv::Mat cloneCropped(clonedImage, RegionOfInterest);
				cv::Mat cropped;
				cloneCropped.copyTo(cropped);
				ObjectAndLocation* varforBestMatch = addObject("adding objects", 0, 0, width, 0, width, length, 0, length, cropped);
				training_images.FindBestMatch(varforBestMatch); 
			
		}
		i++;
	}
	//imshow(filename, clonedImage);
}
...
{
	Mat objectImage = otherObject->image;
	Mat resizedImage1;
	resize(image, resizedImage1, Size(objectImage.cols, objectImage.rows));
	
	Mat space;
	space.create(objectImage.cols - resizedImage1.cols + 1, objectImage.rows - resizedImage1.rows + 1, CV_32FC1);
	matchTemplate(objectImage, resizedImage1, space, cv::TM_CCORR_NORMED);
	double min_val, max_val;
	Point min_loc, max_loc;
	minMaxLoc(space, &min_val, &max_val);
	cout << min_val << " ";
